...
BEGIN TRY
    ...
    /* The actual v1.7 predecessor already owns this index. Reuse it only when
       its complete structural definition is exactly the intended definition. */
    IF NOT EXISTS
    (
        SELECT 1
        FROM sys.indexes i
        WHERE i.object_id = OBJECT_ID(N'dbo.Teams')
          ...
          (
              ...
              JOIN sys.columns c ON c.object_id = ic.object_id AND c.column_id = ic.column_id
              WHERE ic.object_id = i.object_id AND ic.index_id = i.index_id
                AND ic.key_ordinal = 2 AND ic.is_descending_key = 0
                AND c.name = N'TeamCode'
          )
    )
        THROW 53008, N'Development compatibility failed: existing UX_Teams_Organization_TeamCode definition differs from UNIQUE(OrganizationId, TeamCode).', 1;
