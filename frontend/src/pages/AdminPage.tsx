...
function Users({busy,setBusy,msg,setMsg}:{busy:boolean;setBusy:(v:boolean)=>void;msg:string;setMsg:(v:string)=>void}){
 const[teams,setTeams]=useState<Team[]>([]),[edit,setEdit]=useState<AdminUserAccess|null>(null),[active,setActive]=useState(true),[roles,setRoles]=useState<string[]>([]),[scopes,setScopes]=useState<Array<{teamId:number;isPrimary:boolean}>>([]);
 ...
 return <><PeopleBulkPanel onConfirmed={load}/><div className="card" style={{marginTop:18}}><div className="section-title"><div><h2>人員與權限</h2><div className="sub">同一人可有多角色；小組長可授權多個小組，並指定一個主要小組。</div></div></div>{msg&&<div className="note">{msg}</div>}{query.error&&<div role="alert" className="note danger-note">{query.error}</div>}
 ...
 <div className="table-wrap"><table><thead><tr><th>員編</th><th>姓名</th><th>角色</th><th>小組範圍</th><th>狀態</th><th>操作</th></tr></thead><tbody>{rows.map(u=><tr key={u.userId}><td>{u.employeeNo}</td><td>{u.displayName}</td><td>{u.roles.map(r=>roleLabels[r]||r).join('、')||'—'}</td><td>{u.teamScopes.map(s=>`${s.teamName}${s.isPrimary?' ★':''}`).join('、')||'—'}</td><td>{u.isActive?'啟用':'停用'}</td><td><button className="btn small secondary" onClick={()=>open(u)}>維護</button></td></tr>)}</tbody></table></div><Pagination {...query.data} page={query.page} pageSize={query.pageSize} busy={query.loading} onPage={query.setPage} onPageSize={query.setPageSize}/></div>
...
function Locations({busy,setBusy,msg,setMsg}:{busy:boolean;setBusy:(v:boolean)=>void;msg:string;setMsg:(v:string)=>void}){
 const[teams,setTeams]=useState<Team[]>([]),[edit,setEdit]=useState<ManagedLocation|null>(null);
...
function Corrections({busy,setBusy,msg,setMsg}:{busy:boolean;setBusy:(v:boolean)=>void;msg:string;setMsg:(v:string)=>void}){
 const[keyword,setKeyword]=useState(''),[status,setStatus]=useState('PendingAdminClose'),[start,setStart]=useState(''),[end,setEnd]=useState('');
