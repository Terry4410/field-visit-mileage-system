...
export default function VisitorPage(){
  ...
  const [date,setDate]=useState(today),[start,setStart]=useState("08:30"),[end,setEnd]=useState("17:10"),[km,setKm]=useState("40.0"),[purpose,setPurpose]=useState(""),[notes,setNotes]=useState("");
  ...
  const [tempSource,setTempSource]=useState("固定拜訪清單"),[tempProjectId,setTempProjectId]=useState(""),[tempVisitTypeId,setTempVisitTypeId]=useState(""),[tempName,setTempName]=useState(""),[tempAddress,setTempAddress]=useState("");
  ...
  return <>
    ...
    {modal==="project"&&<div className="modal" onMouseDown={e=>{if(e.target===e.currentTarget)setModal(null)}}><div className="modal-panel"><h3>從專案加入</h3><div className="note" style={{marginBottom:14}}>專案地點可以使用管理者預先建立的清單，也可以由外訪員自行維護本次拜訪地點。</div><div className="field"><label>專案名稱</label><select value={projectId} onChange={e=>setProjectId(e.target.value)}>{projects.map(p=><option key={p.projectId} value={p.projectId}>{p.projectName}</option>)}</select></div><div className="field"><label>拜訪形式</label><select value={visitTypeId} onChange={e=>setVisitTypeId(e.target.value)}>{visitTypes.map(v=><option key={v.visitTypeId} value={v.visitTypeId}>{v.visitTypeName}</option>)}</select></div>{isListMode(selectedProject?.locationMode)?<><div className="field"><label>專案拜訪地點</label><select value={projectLocId} onChange={e=>setProjectLocId(e.target.value)}>{projectLocs.map(l=><option key={l.locationId} value={l.locationId}>{l.locationName}｜{l.city||""}{l.district||""}</option>)}</select></div><div className="note">此專案使用管理者維護的固定地點清單。</div></>:<><div className="field"><label>地點名稱</label><input value={manualName} onChange={e=>setManualName(e.target.value)} placeholder="請輸入本次拜訪地點名稱"/></div><div className="field"><label>地址或 Plus Code</label><input value={manualAddress} onChange={e=>setManualAddress(e.target.value)} placeholder="完整地址或 Plus Code"/></div><div className="note">此專案允許外訪員自行維護地點；新增後會標記為待小組長確認。</div></>}<div className="actions" style={{marginTop:16}}><button className="btn" onClick={addProject}>加入行程</button><button className="btn outline" onClick={()=>setModal(null)}>取消</button></div></div></div>}
