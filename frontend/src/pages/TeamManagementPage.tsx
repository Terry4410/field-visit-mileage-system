...
import{api}from"../api";
import type{AdminUserAccess,ManagedTeam,TeamScope}from"../types";

export default function TeamManagementPage(){
 const[teams,setTeams]=useState<ManagedTeam[]>([]),[users,setUsers]=useState<AdminUserAccess[]>([]),[selectedTeamId,setSelectedTeamId]=useState<number|null>(null);
 ...
 const load=async()=>{
  const[t,u]=await Promise.all([api<ManagedTeam[]>("/admin/teams?includeInactive=true"),api<AdminUserAccess[]>("/admin/users")]);
 ...
 const normalizeScopes=(scopes:TeamScope[])=>scopes.map(s=>({teamId:s.teamId,isPrimary:s.isPrimary}));
 const saveAccess=async(u:AdminUserAccess,scopes:Array<{teamId:number;isPrimary:boolean}>)=>{
  await api(`/admin/users/${u.userId}/access`,{
   method:"PUT",
   body:JSON.stringify({isActive:u.isActive,roles:u.roles,teamScopes:scopes})
  });
 };
 const toggleMember=async(u:AdminUserAccess,checked:boolean)=>{
  if(!selectedTeam)return;
  const current=normalizeScopes(u.teamScopes);
  let next=current;
  if(checked){
   ...
   if(u.roles.some(r=>r.toLowerCase()==="leader")&&next.length===0){
    return setMsg(`${u.displayName} 具有「小組長」角色且只有此小組；請先在人員與權限移除小組長角色，或先加入另一個小組。`);
   }
  }
  setBusy(true);setMsg("");
  try{await saveAccess(u,next);setMsg(`${u.displayName} 的小組成員設定已更新；該使用者需重新登入取得最新權限。`);await load()}
  catch(e){setMsg(e instanceof Error?e.message:"成員更新失敗")}finally{setBusy(false)}
 };
 const setPrimary=async(u:AdminUserAccess)=>{
  if(!selectedTeam)return;
  const current=normalizeScopes(u.teamScopes);
  ...
  setBusy(true);setMsg("");
  try{await saveAccess(u,next);setMsg(`${u.displayName} 的主要小組已更新；該使用者需重新登入取得最新權限。`);await load()}
 ...
 return <>
  <div className="grid cols-2">
   <div className="card">
    <div className="section-title"><div><h2>{edit?"修改小組":"新增小組"}</h2><div className="sub">v1.6.1 UAT Fix：小組主檔採新增／修改／停用，不做硬刪除。</div></div>{edit&&<button className="btn small outline" onClick={reset}>取消修改</button>}</div>
   ...
   <div className="card">
    ...
    <div className="table-wrap"><table><thead><tr><th>代碼</th><th>名稱</th><th>狀態</th><th>成員數</th><th>操作</th></tr></thead><tbody>{teams.map(t=>{
     const count=users.filter(u=>u.teamScopes.some(s=>s.teamId===t.teamId)).length;
