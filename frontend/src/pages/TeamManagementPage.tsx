...
import{api}from"../api";
import type{ManagedTeam,V180PersonRow}from"../types";
...
export default function TeamManagementPage(){
 ...
 const saveAccess=async(u:V180PersonRow,teamMemberships:Array<{teamId:number;isPrimary:boolean}>)=>{
  ...
  await api(`/admin/v180/people/${u.employmentId}/access`,{
   method:"PUT",
   body:JSON.stringify({
    roles:v180RoleCodes(u),
    teamMemberships,
 ...
 const toggleMember=async(u:V180PersonRow,checked:boolean)=>{
  ...
  try{
   ...
   setMsg(`${u.displayName} 的小組成員設定已更新；畫面已重新載入最新版本。該使用者需重新登入取得最新權限。`);
   await load();
 ...
 const setPrimary=async(u:V180PersonRow)=>{
  ...
  try{
   await saveAccess(u,next);
   setMsg(`${u.displayName} 的主要小組已更新；畫面已重新載入最新版本。該使用者需重新登入取得最新權限。`);
   await load();
 ...
 return <>
  ...
  <div className="card" style={{marginTop:18}}>
   <div className="section-title"><div><h2>小組成員維護{selectedTeam?`｜${selectedTeam.teamCode} ${selectedTeam.teamName}`:""}</h2><div className="sub">v1.8：以 Employment 與有效 TeamMembership 為唯一成員權限來源；儲存採版本控制並同步相容投影。</div></div>
   ...
   {!selectedTeam?<div className="empty">請先選擇小組。</div>:<>
    <div className="sub" style={{marginBottom:10}}>僅顯示 Internal Employment；External Supervisor 依 Data Scope 管理，不可加入一般小組。★ 代表主要小組。</div>
    <div className="grid cols-2"><label>成員搜尋<input value={personKeyword} placeholder="工號、姓名或 Email" onChange={e=>setPersonKeyword(e.target.value)}/></label><label className="check-row"><input type="checkbox" checked={onlyMembers} onChange={e=>setOnlyMembers(e.target.checked)}/>僅顯示此小組成員</label></div>{peopleQuery.error&&<div role="alert" className="note danger-note">{peopleQuery.error}</div>}<div className="table-wrap"><table><thead><tr><th>員編</th><th>姓名</th><th>角色</th><th>{selectedTeam.teamCode} {selectedTeam.teamName}成員</th><th>是否主要小組</th><th>所屬小組</th></tr></thead><tbody>{users.map(u=>{
     const scope=u.teamMemberships.find(s=>s.teamId===selectedTeam.teamId);
     const accountReady=u.adminEnabled!==null&&u.adminEnabled!==undefined;
     return <tr key={u.employmentId}><td>{u.employeeNo||`Employment ${u.employmentId}`}</td><td>{u.displayName}</td><td>{v180RoleCodes(u).join("、")||"—"}</td><td><label className="check-row"><input type="checkbox" checked={!!scope} disabled={busy||!accountReady||(!selectedTeam.isActive&&!scope)} onChange={e=>void toggleMember(u,e.target.checked)}/>{!accountReady?"無登入帳號":scope?"已加入此小組":"加入此小組"}</label></td><td>{scope?<label className="check-row"><input type="radio" name={`primary-${u.employmentId}`} checked={scope.isPrimary} disabled={busy||!accountReady} onChange={()=>void setPrimary(u)}/>{scope.isPrimary?"主要小組":"設為主要"}</label>:"—"}</td><td>{u.teamMemberships.map(s=>`${s.name}${s.isPrimary?" ★":""}`).join("、")||"—"}</td></tr>
