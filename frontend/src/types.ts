...
export interface TripQueryRow{visitTripId:number;tripNo:string;visitDate:string;startTime?:string;endTime?:string;visitorId:number;employeeNo:string;visitorName:string;teamId?:number;teamName?:string;route:string;projectNames:string;visitTypeNames:string;claimedDistanceKm?:number;systemDistanceKm?:number;approvedDistanceKm?:number;ratePerKmSnapshot?:number;subsidyAmount?:number;mileageState:string;status:string;statusName:string;snapshotVersion:number;isSnapshot:boolean;notes?:string;correctionStatus?:string;stops:QueryStop[]}
