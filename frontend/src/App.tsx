...
const navByRole:Record<string,NavItem[]>={
 ...
 admin:[{path:'/admin',label:'管理儀表板',short:'總覽'},{path:'/admin/users',label:'人員與權限',short:'人員'},{path:'/admin/locations',label:'地點主檔',short:'地點'},{path:'/admin/projects',label:'專案與拜訪形式',short:'專案'},{path:'/admin/rates',label:'補助費率',short:'費率'},{path:'/admin/query',label:'行程查詢',short:'查詢'},{path:'/admin/corrections',label:'更正管理',short:'更正'}],
...
export default function App(){
 ...
 return <div className="app-shell">
  <aside className="sidebar"><div className="brand">外訪行程管理<small>Field Visit Mileage System</small></div><div className="role-box"><label>目前登入</label><strong>{user.displayName}</strong><span>{user.employeeNo}｜{roleLabel[role]}｜{teamText}</span>{roles.length>1&&<select className="role-switch" value={role} onChange={e=>switchRole(e.target.value)}>{roles.map(r=><option key={r} value={r}>切換為：{roleLabel[r]||r}</option>)}</select>}</div><nav className="nav">{nav.map(x=><NavLink key={x.path} end={x.path===home} to={x.path}>● {x.label}</NavLink>)}</nav><button className="btn secondary logout-btn" onClick={logout}>登出</button><div className="sidebar-footer">UAT v1.6.0 RC<br/>DB Schema 1.6.0</div></aside>
  <main className="main"><header className="topbar"><div><h1>{title}</h1><div className="top-subtitle">手機優先｜Azure SQL UAT｜v1.6.0 Route Provider：Mock</div></div><div className="user-chip"><strong>{user.displayName}</strong><span>{roleLabel[role]}｜{teamText}</span>{roles.length>1&&<select value={role} onChange={e=>switchRole(e.target.value)}>{roles.map(r=><option key={r} value={r}>{roleLabel[r]||r}</option>)}</select>}<button className="btn small secondary" onClick={logout}>登出</button></div></header><section className="content"><Routes>
   ...
   <Route path="/admin" element={role==='admin'?<AdminPage section="dashboard"/>:<Navigate to={home}/>}/><Route path="/admin/users" element={role==='admin'?<AdminPage section="users"/>:<Navigate to={home}/>}/><Route path="/admin/locations" element={role==='admin'?<AdminPage section="locations"/>:<Navigate to={home}/>}/><Route path="/admin/projects" element={role==='admin'?<AdminPage section="projects"/>:<Navigate to={home}/>}/><Route path="/admin/rates" element={role==='admin'?<AdminPage section="rates"/>:<Navigate to={home}/>}/><Route path="/admin/query" element={role==='admin'?<UnifiedQueryPage/>:<Navigate to={home}/>}/><Route path="/admin/corrections" element={role==='admin'?<AdminPage section="corrections"/>:<Navigate to={home}/>}/>
