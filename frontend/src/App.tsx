...
export default function App(){
 ...
 return <div className="app-shell">
  ...
  <main className="main"><header className="topbar"><div><h1>{title}</h1><div className="top-subtitle">手機優先｜UAT Pilot｜v1.7.2｜Route Provider：Mock</div></div><div className="user-chip"><strong>{user.displayName}</strong><span>{roleLabel[role]}｜{teamText}</span>{roles.length>1&&<select value={role} onChange={e=>switchRole(e.target.value)}>{roles.map(r=><option key={r} value={r}>{roleLabel[r]||r}</option>)}</select>}<button className="btn small secondary" onClick={logout}>登出</button></div></header><section className="content"><Routes>
   ...
   <Route path="/admin" element={role==='admin'?<AdminPage section="dashboard"/>:<Navigate to={home}/>}/><Route path="/admin/users" element={role==='admin'?<V180PeopleAdminPage/>:<Navigate to={home}/>}/><Route path="/admin/teams" element={role==='admin'?<TeamManagementPage/>:<Navigate to={home}/>}/><Route path="/admin/locations" element={role==='admin'?<AdminPage section="locations"/>:<Navigate to={home}/>}/><Route path="/admin/projects" element={role==='admin'?<AdminPage section="projects"/>:<Navigate to={home}/>}/><Route path="/admin/visit-types" element={role==='admin'?<AdminPage section="visit-types"/>:<Navigate to={home}/>}/><Route path="/admin/rates" element={role==='admin'?<AdminPage section="rates"/>:<Navigate to={home}/>}/><Route path="/admin/query" element={role==='admin'?<UnifiedQueryPage/>:<Navigate to={home}/>}/><Route path="/admin/corrections" element={role==='admin'?<AdminPage section="corrections"/>:<Navigate to={home}/>}/>
