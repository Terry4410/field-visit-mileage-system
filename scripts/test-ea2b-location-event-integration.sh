...
cd "$repo_root"
image="${EA2B_SQLSERVER_IMAGE:-mcr.microsoft.com/mssql/server:2022-latest}"
container_name="fieldvisit-ea2b-${GITHUB_RUN_ID:-local}-$$"
sa_password='Ea2b!LocationEvent2026_TestOnly'
database_name="FieldVisitEA2B_${GITHUB_RUN_ID:-local}_$$"
sqlcmd_path=''
cleanup() { docker rm -f "$container_name" >/dev/null 2>&1 || true; }
trap cleanup EXIT INT TERM
fail() { echo "EA2B-REAL-SQL-FAIL: $*" >&2; exit 1; }
command -v docker >/dev/null 2>&1 || fail "Docker is required."
command -v dotnet >/dev/null 2>&1 || fail ".NET SDK is required."
echo "EA2B_SQL_IMAGE=$image"
...
echo "EA2B_SQLSERVER_VERSION=$("${sqlcmd[@]}" -d master -h -1 -W -Q "SET NOCOUNT ON; SELECT CAST(SERVERPROPERTY('ProductVersion') AS nvarchar(128));" | tr -d '\r' | tail -n1)"
...
EA2B_SQL_CONNECTION="$connection_string" \
  dotnet run --project backend/tests/FieldVisit.EA2B.Integration.Tests/FieldVisit.EA2B.Integration.Tests.csproj --configuration Release
