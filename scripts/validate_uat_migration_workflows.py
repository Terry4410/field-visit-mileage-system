...
STAGE_007_PERMISSION_SCRIPTS = {
    "grant": ROOT / "database" / "migrations" / "security" / "uat" / "Grant-gh-fieldvisit-uat-migrate-1800_007.sql",
    "verify": ROOT / "database" / "migrations" / "security" / "uat" / "Verify-gh-fieldvisit-uat-migrate-1800_007.sql",
    "revoke": ROOT / "database" / "migrations" / "security" / "uat" / "Revoke-gh-fieldvisit-uat-migrate-1800_007.sql",
}

STAGE_007_PARTIAL_MARKERS = (
    "object_id(n'dbo.geocodingattempts',n'u')",
    "object_id(n'dbo.routecalculationattempts',n'u')",
    "object_id(n'dbo.mileagegovernanceevents',n'u')",
    "col_length(n'dbo.locations',n'selectedgeocodingattemptid')",
    "col_length(n'dbo.mileagecalculations',n'selectedroutecalculationattemptid')",
    "col_length(n'dbo.mileagecalculations',n'manualfallbackused')",
    "col_length(n'dbo.mileagecalculations',n'distancedecisiongovernanceversion')",
    "col_length(n'dbo.mileagecalculations',n'approveddistancesource')",
    "col_length(n'dbo.mileagecalculations',n'approvalbasiscode')",
    "col_length(n'dbo.mileagecalculations',n'approvalbasishash')",
    "col_length(n'dbo.mileagecalculations',n'distanceapprovedat')",
    "col_length(n'dbo.mileagecalculations',n'distanceapprovedbyuserid')",
    "col_length(n'dbo.mileagecalculations',n'invalidatedat')",
    "col_length(n'dbo.mileagecalculations',n'invalidatedbyuserid')",
    "col_length(n'dbo.mileagecalculations',n'invalidationreason')",
    "col_length(n'dbo.visittripsnapshots',n'mileagerouteattemptidsnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'routetravelmodesnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'routecalculatedatsnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'routecalculationstatussnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'routeerrorcodesnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'routecorrelationidsnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'approveddistancesourcesnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'approvalbasiscodesnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'approvalbasishashsnapshot')",
    "col_length(n'dbo.visittripsnapshots',n'distanceapprovedatsnapshot')",
    "object_id(n'dbo.tr_locations_selectedgeocodingattempt',n'tr')",
    "object_id(n'dbo.tr_mileagecalculations_selectedroutetrip',n'tr')",
    "object_id(n'dbo.tr_mileagecalculations_decisionevidence',n'tr')",
    "object_id(n'dbo.tr_visittripsnapshots_routeattempttrip',n'tr')",
)

STAGE_007_FORBIDDEN_UPDATE_OBJECTS = {
    "Organizations",
    "Teams",
    "UserIdentityProfiles",
    "Locations",
...
def validate_stage_007_permission_tooling() -> None:
    workflow_path = ROOT / ".github" / "workflows" / "azure-sql-uat-migration-1800-007.yml"
    workflow = workflow_path.read_text(encoding="utf-8")
    workflow_compact = compact_sql(workflow)
    parsed = yaml.load(workflow, Loader=yaml.BaseLoader)
    require(isinstance(parsed, dict), "007 workflow: YAML root must be a mapping")
    steps = parsed.get("jobs", {}).get("migrate-1800_007", {}).get("steps", [])
    require(isinstance(steps, list), "007 workflow: migration steps must be a list")

    def exact_step(name: str) -> dict[str, str]:
        matches = [step for step in steps if step.get("name") == name]
        require(len(matches) == 1, f"007 workflow: step missing or duplicated: {name}")
        return matches[0]

    def step_run(name: str) -> str:
        run = exact_step(name).get("run", "")
        require(isinstance(run, str) and run, f"007 workflow: step has no run block: {name}")
        return run

    def validate_token_masking(name: str, run: str) -> None:
        require(
            run.count("az account get-access-token") == 1,
            f"007 workflow: SQL step must obtain exactly one Azure SQL token: {name}",
        )
        require(
            run.count('Write-Output "::add-mask::$token"') == 1,
            f"007 workflow: token masking missing or duplicated: {name}",
        )
    ...
    def validate_timeout_profile(name: str, connection_timeout: int, query_timeout: int) -> str:
        ...
        require(
            len(invocations) == 1,
            f"007 workflow: expected exactly one Invoke-Sqlcmd in step: {name}",
        )
        invocation = invocations[0]
        require(
            re.findall(r"-ConnectionTimeout\s+(\d+)", invocation) == [str(connection_timeout)],
            f"007 workflow: incorrect ConnectionTimeout in step: {name}",
        )
        require(
            re.findall(r"-QueryTimeout\s+(\d+)", invocation) == [str(query_timeout)],
            f"007 workflow: incorrect QueryTimeout in step: {name}",
        )
        validate_token_masking(name, run)
        return run

    scripts: dict[str, str] = {}
    for kind, path in STAGE_007_PERMISSION_SCRIPTS.items():
        require(path.is_file(), f"007: {kind} permission script is missing")
        scripts[kind] = path.read_text(encoding="utf-8")
    ...
    for kind, source in scripts.items():
        ...
        require(
            ...
            and {permission.lower() for permission in explicit_permission_allowlist}
            == expected_explicit_permission_allowlists[kind],
            f"007 {kind}: explicit permission allowlist is not exact or permits DELETE",
        )
        require(
            re.search(r"\bGRANT\s+DELETE\b", source, re.IGNORECASE) is None,
            f"007 {kind}: DELETE permission grant is forbidden",
        )
        require(source.count("@CanDeleteDatabase <> 0") == 1,
                f"007 {kind}: database DELETE fail-closed gate missing or duplicated")
        require(source.count("@CanDeleteDboSchema <> 0") == 1,
                f"007 {kind}: dbo schema DELETE fail-closed gate missing or duplicated")
    ...
    for kind, source in scripts.items():
        for role in broad_roles:
            ...
            require(
                len(re.findall(pattern, source, re.IGNORECASE)) == 1,
                f"007 {kind}: fail-closed {role}=NO gate missing or duplicated",
            )
    ...
    require(role_additions["grant"] == ["db_ddladmin"],
            "007 grant: db_ddladmin must be the only role addition")
    require(not role_additions["verify"] and not role_additions["revoke"],
            "007 verify/revoke: role additions are forbidden")

    require(len(STAGE_007_PARTIAL_MARKERS) == 29,
            "007 validator: frozen partial-state marker count must be exactly 29")
    for marker in STAGE_007_PARTIAL_MARKERS:
        require(grant_compact.count(marker) == 1,
                f"007 grant: partial-state marker missing or duplicated: {marker}")
        require(workflow_compact.count(marker) == 1,
                f"007 workflow: partial-state marker missing or duplicated: {marker}")
    ...
    require(granted_update_matches == ["MileageCalculations"],
            "007 grant: exact UPDATE grant set must be {MileageCalculations}")
    require(revoked_update_matches == ["MileageCalculations"],
            "007 revoke: exact UPDATE revoke set must be {MileageCalculations}")
    ...
    required_grant_fragments = (
        ...
        "GRANT_PREPARED_FOR_1800_007",
        "latest SchemaVersion is not exact predecessor 1.8.0-006",
        "one or more of 29 Stage 007 partial-state markers exist",
        "pre-stage UPDATE or DELETE capability residue exists",
    )
    for fragment in required_grant_fragments:
        require(fragment in grant, f"007 grant: required fail-closed fragment missing: {fragment}")

    required_verify_fragments = (
        "1800_007_PERMISSION_GATE",
        "@CanAlterLocations <> 1",
        "@CanAlterMileageCalculations <> 1",
        "@CanAlterVisitTripSnapshots <> 1",
        "@CanReferenceLocations <> 1",
        "@CanReferenceUsers <> 1",
        "@CanReferenceVisitTrips <> 1",
        "@CanReferenceVisitTripSnapshots <> 1",
        "@CanUpdateMileageCalculations <> 1",
        "@CanUpdateOrganizations <> 0",
        "@CanUpdateTeams <> 0",
        "@CanUpdateUserIdentityProfiles <> 0",
        "@CanUpdateLocations <> 0",
    ...
    for fragment in required_verify_fragments:
        require(fragment in verify, f"007 verify: required fail-closed gate missing: {fragment}")
    ...
    required_revoke_fragments = (
        ...
        "ALTER ROLE db_ddladmin DROP MEMBER [gh-fieldvisit-uat-migrate]",
        "@CanInsertDboSchema <> 0",
        "@CanUpdateMileageCalculations <> 0",
        "@CanUpdateOrganizations <> 0",
        "@CanUpdateTeams <> 0",
        "@CanUpdateUserIdentityProfiles <> 0",
        "@CanUpdateLocations <> 0",
        ...
        "@CanUpdateMileageRateRules <> 0",
        "@CanUpdateEmployments <> 0",
        "REVOKED_1800_007_ELEVATION",
    )
    for fragment in required_revoke_fragments:
        require(fragment in revoke, f"007 revoke: required fail-closed check missing: {fragment}")
    require("SchemaVersions" not in revoke,
            "007 revoke: revoke must remain independently usable without a SchemaVersion gate")

    require(
        verify_compact.count(
            "object_name(p.major_id)=n'mileagecalculations'andp.permission_name=n'update'andp.state=n'g'"
        ) == 1,
        "007 verify: exact MileageCalculations UPDATE allowlist is missing or duplicated",
    )
    require(
        "object_name(p.major_id)=n'mileagecalculations'" not in revoke_compact,
        "007 revoke: CONNECT-only baseline check must not allow MileageCalculations UPDATE",
    )
    ...
    require(required_update_matches == ["mileagecalculations"],
            "007 workflow: required UPDATE set must be exactly {MileageCalculations}")
    require(
        len(forbidden_update_matches) == len(STAGE_007_FORBIDDEN_UPDATE_OBJECTS)
        and set(forbidden_update_matches) == {
            name.lower() for name in STAGE_007_FORBIDDEN_UPDATE_OBJECTS
        },
        "007 workflow: forbidden prior-stage UPDATE set is not exact",
    )
    require(
        len(alter_matches) == 3
        and set(alter_matches) == {"locations", "mileagecalculations", "visittripsnapshots"},
        "007 workflow: required ALTER set is not exact",
    )
    require(
        len(reference_matches) == 4
        and set(reference_matches) == {"locations", "users", "visittrips", "visittripsnapshots"},
        "007 workflow: required REFERENCES set is not exact",
    )

    warmup_name = "Warm up primary Azure SQL connectivity"
    preflight_name = "Validate predecessor, clean state, exact permissions, and capture fingerprints"
    up_name = "Apply only 1800_007 Up.sql"
    verify_name = "Run exact 1800_007 Verify.sql"
    history_name = "Revalidate frozen v1.7.2 historical SHA-256 fingerprints"
    final_name = "Confirm exact result, historical fingerprints, and STOP_FOR_REVIEW"
    expected_sql_steps = {
        warmup_name,
    ...
    warmup = validate_timeout_profile(warmup_name, 60, 60)
    query_matches = re.findall(r"(?m)^\s*\$query\s*=\s*'([^'\r\n]*)'\s*$", warmup)
    require(len(query_matches) == 1, "007 workflow: warm-up must have one literal $query assignment")
    normalized_query = re.sub(r"\s+", " ", query_matches[0]).strip()
    approved_query = (
        "SET NOCOUNT ON; SELECT DB_NAME() AS DatabaseName, "
        "USER_NAME() AS DatabasePrincipal;"
    )
    require(normalized_query == approved_query,
            "007 workflow: warm-up query is not the exact read-only query")
    require(
        not re.search(
            r"(?i)\b(?:INSERT|UPDATE|DELETE|MERGE|CREATE|ALTER|DROP|TRUNCATE|EXEC(?:UTE)?|FROM|JOIN)\b|dbo\.",
            normalized_query,
        ),
        "007 workflow: warm-up query contains business-table SQL, DDL, or DML",
    )
    for forbidden in ("-InputFile", "Up.sql", "Verify.sql"):
        require(forbidden not in warmup, f"007 workflow: warm-up contains forbidden token: {forbidden}")
    require("if ($databaseName -ne 'db-fieldvisit-uat') { throw" in warmup,
            "007 workflow: warm-up exact database fail-closed comparison missing")
    require("if ($databasePrincipal -ne 'gh-fieldvisit-uat-migrate') { throw" in warmup,
            "007 workflow: warm-up exact principal fail-closed comparison missing")
    require(warmup.count("$maxAttempts = 3") == 1,
            "007 workflow: warm-up maxAttempts must be exactly 3")
    require(warmup.count("$backoffSeconds = 5") == 1,
            "007 workflow: warm-up backoffSeconds must be exactly 5")
    require(
        len(re.findall(
            r"for\s*\(\s*\$attempt\s*=\s*1\s*;\s*\$attempt\s*-le\s*\$maxAttempts\s*;\s*\$attempt\+\+\s*\)",
            warmup,
        )) == 1,
        "007 workflow: warm-up retry loop must run through maxAttempts",
    )
    require(warmup.count("Start-Sleep -Seconds $backoffSeconds") == 1,
            "007 workflow: warm-up retry sleep is missing or duplicated")
    require(
        len(re.findall(
            r"if\s*\(\s*\$attempt\s*-ge\s*\$maxAttempts\s*\)\s*\{\s*throw\s*\}",
            warmup,
        )) == 1,
        "007 workflow: warm-up final failed attempt must throw",
    )
    ...
    require(sql_step_names == expected_sql_steps,
            "007 workflow: SQL execution steps differ from the frozen connectivity contract")
    require(re.search(r"(?i)\bApplicationIntent\b", workflow) is None,
            "007 workflow: ApplicationIntent must not be used")
    require(re.search(r"(?i)-ConnectionTimeout\s+15\b", workflow) is None,
            "007 workflow: all SQL connections must use ConnectionTimeout 60")

    up = ROOT / "database" / "migrations" / "1800_007_mileage_google_governance" / "Up.sql"
    stage_verify = ROOT / "database" / "migrations" / "1800_007_mileage_google_governance" / "Verify.sql"
    history = ROOT / "database" / "migrations" / "security" / "uat" / "Verify-v180-historical-fingerprints.sql"
    require(sha256(up) == "d89a061c15c95427a0cb1793b762a7fce306cd8019f276bb26664f617ffa364d",
            "007: frozen Up.sql SHA-256 changed")
    require(sha256(stage_verify) == "c27e43aa0311be1c8f2c7f8a424ca8fff80faaac7a311335d56f69b0a72e395e",
            "007: frozen Verify.sql SHA-256 changed")
    require(sha256(history) == "1d7f77dc200590c2ba3b989ed340442d835f8e02be9edbb57af68ae5e4e9cba0",
            "007: frozen historical verifier SHA-256 changed")

    governance = GOVERNANCE.read_text(encoding="utf-8")
    governance_flat = re.sub(r"\s+", " ", governance)
    require(
        governance.count("| 1800_007 | `dbo.MileageCalculations` |") == 1,
        "007 governance: exact temporary UPDATE row missing or duplicated",
    )
    require(
        "No `UPDATE` permission is permitted on `dbo.Organizations`, `dbo.Teams`, `dbo.UserIdentityProfiles`, `dbo.Locations`, `dbo.DeploymentSiteLocationAssignments`, `dbo.Projects`, `dbo.VisitTypes`, `dbo.MileageRateRules`, or `dbo.Employments`"
        in governance_flat,
        "007 governance: exact forbidden UPDATE statement missing",
    )
    require(
        "The Stage 007 Grant and Revoke scripts are independent approved actions outside the migration workflow"
        in governance_flat,
        "007 governance: independent permission-action rule missing",
    )
    require(
        "The immutable Stage 007 SQL locks, 29-marker clean-state gate, recovery governance, single-stage execution boundary, no-automatic-rerun rule, and terminal `STOP_FOR_REVIEW` remain unchanged."
        in governance_flat,
        "007 governance: immutable/recovery/single-stage/STOP_FOR_REVIEW preservation missing",
    )

    print(
        "PASS 1800_007 permission-tooling+no-DELETE+broad-role-gates+29-markers+"
        "MileageCalculations-only-UPDATE+step-bound-connectivity+immutable-hashes"
    )
...
def main() -> int:
    try:
        ...
        validate_stage_007_permission_tooling()
