...
cd "$repo_root"
image="${EA2C_SQLSERVER_IMAGE:-mcr.microsoft.com/mssql/server:2022-latest}"
container_name="fieldvisit-ea2c-${GITHUB_RUN_ID:-local}-$$"
sa_password='Ea2c!ImportCompleted2026_TestOnly'
database_name="FieldVisitEA2C_${GITHUB_RUN_ID:-local}_$$"
sqlcmd_path=''
cleanup() { docker rm -f "$container_name" >/dev/null 2>&1 || true; }
trap cleanup EXIT INT TERM
fail() { echo "EA2C-REAL-SQL-FAIL: $*" >&2; exit 1; }
command -v docker >/dev/null 2>&1 || fail "Docker is required."
command -v dotnet >/dev/null 2>&1 || fail ".NET SDK is required."
echo "EA2C_SQL_IMAGE=$image"
...
for _ in $(seq 1 90); do
  ...
  if [[ -n "$sqlcmd_path" ]]; then
    ...
    if "${sqlcmd[@]}" -d master -Q 'SET NOCOUNT ON; SELECT 1;' >/dev/null 2>&1; then ready=1; break; fi
  fi
  sleep 1
done
...
echo "EA2C_SQLSERVER_VERSION=$("${sqlcmd[@]}" -d master -h -1 -W -Q "SET NOCOUNT ON; SELECT CAST(SERVERPROPERTY('ProductVersion') AS nvarchar(128));" | tr -d '\r' | tail -n1)"
...
EA2C_SQL_CONNECTION="$connection_string" \
  dotnet run --project backend/tests/FieldVisit.EA2C.Integration.Tests/FieldVisit.EA2C.Integration.Tests.csproj --configuration Release
