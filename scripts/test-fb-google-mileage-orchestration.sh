...
image="${FB_SQLSERVER_IMAGE:-mcr.microsoft.com/mssql/server:2022-latest}"
container_name="fieldvisit-fb-${GITHUB_RUN_ID:-local}-$$"
sa_password='Fb!GoogleMileage2026_TestOnly'
database_name="FieldVisitFB_${GITHUB_RUN_ID:-local}_$$"
sqlcmd_path=''

cleanup(){ docker rm -f "$container_name" >/dev/null 2>&1 || true; }
trap cleanup EXIT INT TERM
fail(){ echo "FB-REAL-SQL-FAIL: $*" >&2; exit 1; }
...
for _ in $(seq 1 90); do
  "${sqlcmd[@]}" -d master -Q 'SET NOCOUNT ON; SELECT 1;' >/dev/null 2>&1 && break
  sleep 1
done
...
FB_SQL_CONNECTION="Server=127.0.0.1,$port;Database=$database_name;User Id=sa;Password=$sa_password;Encrypt=True;TrustServerCertificate=True;MultipleActiveResultSets=False" \
  dotnet run --project backend/tests/FieldVisit.FB.Integration.Tests/FieldVisit.FB.Integration.Tests.csproj --configuration Release
