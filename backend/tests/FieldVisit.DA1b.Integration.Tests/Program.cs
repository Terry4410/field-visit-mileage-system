...
VALUES({id},{org},{team},{"L"+id},{name},{"Official"},{city},{district},{address},{plus},0,{"Approved"},{"Success"},{active},SYSUTCDATETIME(),CONVERT(datetime2(3),'2000-01-01T00:00:00'),{tax})");
    ...
    private static async Task AlreadyInactiveNoOp(){await Reset();await using var db=Db();await SeedAuthority(db);await SeedLocation(db,1);await Repo(db).DeactivateManagedLocationAsync(Admin(),1,await Rv(db,1),default);var before=await State(1);var count=(await Audits(1,"LocationDeactivate")).Count;await Repo(db).DeactivateManagedLocationAsync(Admin(),1,Convert.ToBase64String(before.RowVersion),default);StateEq(before,await State(1));Eq(count,(await Audits(1,"LocationDeactivate")).Count);}
    private static async Task AlreadyInactiveStale(){await Reset();await using var db=Db();await SeedAuthority(db);await SeedLocation(db,1);var stale=await Rv(db,1);await Repo(db).DeactivateManagedLocationAsync(Admin(),1,stale,default);var before=await State(1);var count=(await Audits(1,"LocationDeactivate")).Count;await Throws<DbUpdateConcurrencyException>(()=>Repo(db).DeactivateManagedLocationAsync(Admin(),1,stale,default));StateEq(before,await State(1));Eq(count,(await Audits(1,"LocationDeactivate")).Count);}
